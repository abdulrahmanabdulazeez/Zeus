...
void Port_Scanner_Single(){
        ...
        if(WSAStartup(MAKEWORD(2,2),&wd)!=0){
            ...
            cout << yellow << "\n[" << red << "~" << yellow << "]" << green << " Enter the port you want to scan: " << reset;
            int single_Port;
            cin >> single_Port;
        ...
        if((soc=socket(AF_INET, SOCK_STREAM, IPPROTO_TCP))==INVALID_SOCKET){
            ...
            //cout << "[*]Socket creation successful!\n\n";
                ...
                serv.sin_port=htons(single_Port);

                cout << reset << cyan << "|----------------------------|-----------------------------|\n" << reset;


                if(connect(soc,(struct sockaddr*)&serv, sizeof(serv))<0){
                    cout << cyan << "|           " << yellow << single_Port << red << "                          Closed                 \n";
                }

                else{
                    Sleep(900);
                    cout << cyan << "|           " << yellow << single_Port << green << "                          Open                   \n";
                }
...
void Port_Scanner_Multiple(){
        ...
        if(WSAStartup(MAKEWORD(2,2),&wd)!=0){
            ...
            cout << yellow << "\n[" << red << "~" << yellow << "]" << green << " Enter port number to begin scan from: " << reset;
            int startPort;
            cin >> startPort;

            cout << yellow << "[" << red << "~" << yellow << "]" << green << " Enter port number to stop scan when reached: " << reset;
            int stopPort;
            cin >> stopPort;
                ...
                cout << yellow << "[" << white << "!" << yellow << "]" << green << " Starting scan from port " << red << startPort << green << " to port " << red << stopPort << "\n\n";
        ...
        if((soc=socket(AF_INET, SOCK_STREAM, IPPROTO_TCP))==INVALID_SOCKET){
            ...
            //cout << "[*]Socket creation successful!\n\n";
                ...
                if(connect(soc,(struct sockaddr*)&serv, sizeof(serv))<0){
                    cout << cyan << "|           " << yellow << i << red << "                          Closed                 \n";
                }

                else{
                    Sleep(900);
                    cout << cyan << "|           " << yellow << i << green << "                          Open                   \n";
                }
            }
...
void header_Windows(),header_Unix(),Prompt(),AboutTool(),Author_Stuffs(),Scan_Type(),Clear_Scr();
void fix_scan();
...
int main(){
    ...
    cout << yellow << "[" << red << "1" << yellow << "]" << blue << " Scan a host.\n";
...
void Scan_Type(){
    ...
    cout << yellow << "\n\n[" << red << "1" << yellow << "]" << blue << " Scan a single port.\n";
    cout << yellow << "[" << red << "2" << yellow << "]" << blue << " Scan multiple ports(Loop Scan).\n";
    cout << yellow << "[" << red << "3" << yellow << "]" << blue << " Scan well know ports.\n";
    cout << yellow << "[" << red << "4" << yellow << "]" << blue << " Exit.\n";

    cout << yellow << "\n[" << red << "~" << yellow << "]" << cyan << " Select scan type: ";
    cout << reset;
    string Sc_Type;
    cin >> Sc_Type;
...
void fix_scan(){
    cout << "\n\n|+| This option scans the state of well known and common ports.";
    cout << "\n|+| The ports scanned by this option are: \n";
    cout << "|+| 20,21,22,23,25,53,67,68,80,110,119,123,143,161,194,443.\n";

    cout << red << "\n[" << cyan << "!" << red << "]" << green << " Do you want to continue[yes/no]: ";

    string quest;
    cout << reset;
    cin >> quest;

        if(quest=="yes"||quest=="YES"||quest=="Yes"){
            Clear_Scr();
            Port_Scanner_Fixed();
        }
        else if(quest=="no"||quest=="NO"||quest=="No"){
            Clear_Scr();
            Scan_Type();
        }
        else{
...
void AboutTool(){
    cout << "\n\n Port-Fin(port finder) V2 is a tool which scans for open and\n";
    cout << " closed port on a website/host.\n";
    cout << " With different options available, you can choose to scan a range\n";
    cout << " of ports or scan a single or scan common ports.\n\n";
    cout << " Note that this tool only checks for the open/closed state of a port.!\n\n";
